...
class TensorRTEngine:
    ...
    def __init__(self, engine_path: Path | str):
        """TensorRT 엔진 로드 및 고정 메모리 버퍼 1회 사전 바인딩."""
    ...
    def _allocate_buffers(self):
        """추론 루프 내 메모리 재할당 배제를 위한 Host/Device Pinned Buffer 사전 할당."""
        for i in range(self.engine.num_io_tensors):
            ...
            # TensorRT 10 엔진에 GPU 버퍼 물리 주소 사전 등록 (0-Byte Reallocation)
    ...
    def execute(self, input_data: np.ndarray) -> np.ndarray:
        """비동기 파이프라인(H2D 복사 -> V3 비동기 추론 -> D2H 복사) 실행 및 동기화."""
        ...
        # 비동기 D2H 복사 완료 대기 후 Host 버퍼 반환
        self.stream.synchronize()
        return self.h_output

    def destroy(self):
        """CUDA 스트림 동기화 대기 및 GPU 디바이스 메모리 안전 해제."""

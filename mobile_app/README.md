...
| `POST` | `/api/kiosk/cancel`         | `KioskCancelRequest(bin_id, user_id, reason?)`     | `KioskCancelResponse`    | 배출 세션 중도 취소 (모바일 및 키오스크 양방향 복귀 동기화) |
...
### 2. WebSocket 실시간 정산 및 세션 동기화 이벤트

- **엔드포인트**: `ws://{SERVER_IP}:8000/ws/kiosk/{bin_id}/mobile`
- **수신 이벤트 1 (`RECYCLE_COMPLETE`) 페이로드 예시**:
...
- **수신 이벤트 2 (`SESSION_CANCELLED`) 페이로드 예시**:
  - 키오스크 화면에서 '투입 취소' 버튼을 누르거나 모바일에서 취소할 때 양방향 동기화를 위해 수신됩니다.
...
### 3. 양방향 세션 취소 동기화 시퀀스 (Bidirectional Cancellation Flow)

배출 도중 모바일 앱 또는 Qt 키오스크 화면에서 '투입 취소'를 누를 경우, 중앙 서버를 통해 양방향으로 즉시 상태가 동기화되어 이전 화면으로 안전하게 복귀합니다.
...
sequenceDiagram
    ...
    participant App as 모바일 앱 (Android)
    participant Server as 중앙 서버 (FastAPI)
    participant Kiosk as 스마트 키오스크 (Qt)

    alt Case A: 모바일 바텀시트에서 '투입 취소' 클릭
        User->>App: 바텀시트 [투입 취소] 클릭
        App->>Server: POST /api/kiosk/cancel (bin_id, user_id)
        Server->>Server: 키오스크 세션 초기화 (IDLE 복귀)
        Server-->>Kiosk: WebSocket 푸시: SESSION_CANCELLED
        Server-->>App: 취소 성공 응답 (200 OK)
        Kiosk->>Kiosk: 배출 화면 -> 대기(Idle) 화면 복귀 & 안내
        App->>App: 바텀시트 닫힘 & 홈 상태 초기화
    else Case B: 키오스크 화면에서 '취소/복귀' 버튼 클릭
        User->>Kiosk: 화면 상단 [투입 취소] 버튼 클릭
        Kiosk->>Server: POST /api/kiosk/cancel (bin_id, user_id)
        Server->>Server: 키오스크 세션 초기화 (IDLE 복귀)
        Server-->>App: WebSocket 푸시: SESSION_CANCELLED
        Server-->>Kiosk: 취소 성공 응답 (200 OK)
        Kiosk->>Kiosk: 대기(Idle) 화면 복귀
        App->>App: WebSocket 수신 -> 바텀시트 자동 닫힘 & 스낵바 알림
    end
```
